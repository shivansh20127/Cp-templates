...
int poow(int x, int y, int p) { int res = 1; x = x % p;
...
public:
    vector<int>parent;
    vector<int> size;
    DSU(int n) { parent.resize(n+1); for(int i=0;i<=n;i++) parent[i]=-1; size.resize(maxN,-1); }
    ...
    bool union_set(int a,int b){ int pa=find_set(a); int pb=find_set(b);
        if(pa==pb) { return 0; } if(parent[pa]>parent[pb]) { swap(pa,pb); }
         parent[pb]=pa; size[pa]+=size[pb]; return 1; }
